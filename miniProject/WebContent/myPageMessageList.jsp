...
<body>
	...
	<%
		...
		try{
			...
			sql="select count(*) as cnt from message where m_id='"+userID+"'";
						...
						<div class="col-md-9">
							<div align="left">
								<ul class="nav nav-pills nav-justified">
									<li role="presentation" id="receiveMessage" class="active">
										<a href="#">받은 쪽지함</a>
									</li>
									<li role="presentation" id="sendMessage" class="">
										<a href="myPageMessageList2.jsp">보낸 쪽지함</a>
									</li>
								</ul>
							</div>
							...
							<table border="1" id="table1"
								...
								<tr align="center" valign="middle" bordercolor="#333333">
									...
									<th class="col-md-1 mythstyle">보낸사람</th>
									<th class="col-md-4 mythstyle">제목</th>
									<th class="col-md-1 mythstyle">받은일시</th>
								</tr>
								...
								<%
									try{
										...
										sql += "       FROM ";
										sql += "           (SELECT * ";
										sql += "               FROM message where m_id='"+ userID+"'";//+searchSql;
								...
								%>
											<tr>
												<td><%=Gno %></td>
												<td><%=GuserId %></td>
												<input type="hidden" id="mno<%=i+1%>" name="no" value="<%=GMno%>">
												<td><a id="title<%=i+1%>" class="title"><%=GMtitle %></a></td>
												<td><%=GMdate %></td>					
											</tr>
								<% 
										} //while end
								...
								%>
											<tr>
												<td colspan="4">받은 쪽지가 존재하지 않습니다.</td>
											</tr>
								<%
										}
									} catch(Exception e){
									...
									<ul class="pagination">
									    <%
											if(startPage>showPage) {
												out.print("<li><a href='myPageMessageList.jsp?pageNo="+(startPage-showPage)+"' aria-label='Previous'>");
												out.println("&laquo;</a></li>");
											} 
									    	for(int i = startPage; i<=endPage; i++){
									    		if(nowPageNo==i){
									    			out.println("<li class='active'><a href='myPageMessageList.jsp?pageNo="+i+"'>"+i+"</a></li>");
									    		} else {
									    			out.println("<li><a href='myPageMessageList.jsp?pageNo="+i+"'>"+i+"</a></li>");
									    		}
									    		
									    	}
										    if(endPage<GtotalPage) {
										    	out.print("<li><a href='myPageMessageList.jsp?pageNo="+(startPage+showPage)+"' aria-label='Next'>");
