...
<body>
	...
	<%
		...
		try{
			...
			sql="select count(*) as cnt from bbsreply where u_id='"+userID+"'";
			...
			<div class="myPage-container">
				...
				<div class="row">
					<div id="myPage-nav" class="col-xs-2" align="right">
						<ul class="nav nav-pills nav-stacked">html
							...
							<table border="1" id="table1"
								...
								<%
									try{
										...
										sql += "       FROM ";
										sql += "           (SELECT * ";
										sql += "               FROM bbsreply where u_id='"+ userID+"'";//+searchSql;
										sql += "               ORDER BY br_date DESC ";
										sql += "           ) temp ";
										sql += "   ) ";
										sql += "WHERE rn between " +startRow+ " and "+endRow+ " ORDER BY br_date desc";
										RS=ST.executeQuery(sql);
								...
								%>
											<tr>
												...
												<td><%=GRid %></td>
												<td><%=GRdate %></td>	
												<td><a href="boardDetail.jsp?no=<%=GBno %>">이동하기</a></td>
											</tr>
								<% 
										} //while end
									...
									<ul class="pagination">
									    <%
											if(startPage>showPage) {
												out.print("<li><a href='myPageReplyList.jsp?pageNo="+(startPage-showPage)+"' aria-label='Previous'>");
												out.println("&laquo;</a></li>");
											} 
									    	for(int i = startPage; i<=endPage; i++){
									    		if(nowPageNo==i){
									    			out.println("<li class='active'><a href='myPageReplyList.jsp?pageNo="+i+"'>"+i+"</a></li>");
									    		} else {
									    			out.println("<li><a href='myPageReplyList.jsp?pageNo="+i+"'>"+i+"</a></li>");
									    		}
									    		
									    	}
										    if(endPage<GtotalPage) {
										    	out.print("<li><a href='myPageReplyList.jsp?pageNo="+(startPage+showPage)+"' aria-label='Next'>");
